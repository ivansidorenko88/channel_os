...
function extractDraftContent(message) {
  ...
  if (message.video) return { type: "video", fileId: message.video.file_id, caption: message.caption || null };
  if (message.animation) return { type: "animation", fileId: message.animation.file_id, caption: message.caption || null };
  if (message.document) return { type: "document", fileId: message.document.file_id, caption: message.caption || null };
...
async function createDraftFromMessage(from, message) {
  ...
  return draftRepository.createDraft({ userId: user.id, ...content });
}
