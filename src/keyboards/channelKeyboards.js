function channelSelectKeyboard(channels, draftId) {
  const rows = channels.map((channel) => [{ text: `📢 ${channel.title}`, callback_data: `draft:select_channel:${draftId}:${channel.id}` }]);
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: "🗑 Удалить черновик", callback_data: `draft:delete:${draftId}` }],
